...
public class ItemDAO {
	...
	public static ArrayList<ItemBean> fetchItemById(int itemId)throws SQLException{
		...
		con=getDbConnection();
		try{
			pst=con.prepareStatement("SELECT * FROM item where item_id="+itemId+" ");
			rs=pst.executeQuery();
			while(rs.next()){
	...
	public static boolean InsertItem(ItemBean itemBean) {
		try {
			con=getDbConnection();
			pst=con.prepareStatement("insert into item(item_name,cat_id,item_price) values(?,?,?)");
	...
	public static boolean updateItem(ItemBean itemBean)throws Exception{
		...
		boolean flag=false;
		try {
			pst=con.prepareStatement("update item set item_name=?,item_price=? ,cat_id=? where item_id =?");
			pst.setString(1,itemBean.getItemName());
	...
	public static boolean deleteAllItemByCatId(CategoryBean categoryBean)throws Exception{
		...
		boolean flag=false;
		try {
			pst=con.prepareStatement("delete from item where cat_id =?");
	...
	public static int fetchCategoryIdByItemId(int itemId)throws SQLException{
		...
		con=getDbConnection();
		try{
			pst=con.prepareStatement("SELECT cat_id FROM item where item_id="+itemId+" ");
			rs=pst.executeQuery();
	...
	public static float fetchItemPriceByItemId(int itemId)throws SQLException{
		...
		con=getDbConnection();
		try{
			pst=con.prepareStatement("SELECT item_price FROM item where item_id="+itemId+" ");
			rs=pst.executeQuery();
